...
double N(double x) {
	...
	if(x>=0.0){
		return 1.0-
			...
			*exp(-x*x/2.0)/sqrt(2*pi);
	} else return 1.0 - N(-x);
}

double EurCall::d_plus(double S0,double sigma,double r) {
	return (log(S0/K)+(r + sigma*sigma/2.0)*T)/(sigma*sqrt(T));
}
...
//vega is derivative of price wrt volatility sigma
double EurCall::VegaByBSFormula(double S0,double sigma,double r) {
	double pi = 4.0*atan(1.0);
	return S0*exp(-d_plus(S0,sigma,r)*d_plus(S0,sigma,r)/2.0)*sqrt(T)/sqrt(2*pi);
}